...
def scatter_plot(TureValues,PredictValues,R2, train_mae, train_mse):
    ...
    plt.scatter(TureValues, PredictValues, s=20, c='r', edgecolors='k', marker='o', alpha=0.8,
                label='R^2 = %.2f' % R2)
    plt.xlabel('The real value of POI salience')
    plt.ylabel('The prediction value of POI salience')
    # plt.legend()
...
cn = 1  # When cn is 0, all features ( 7 selected by correlation analysis ) are involved in training.
for i in range(cn):
    p_drop.append(p[i])
...
MLP = MLPRegressor(hidden_layer_sizes=(4, 5), learning_rate_init=0.1, max_iter=400,  random_state=70)
...
#Accuracy evaluation of verify set
df = pd.read_excel('data/verify.xlsx', sheet_name='verify')
p_drop = ['Salience']
x = df.drop(p_drop, axis=1)
x_verify = s.transform(np.array(x))
y_verify = np.array(df['Salience'])
y_verify_pred = MLP_m.predict(x_verify)
verify_R2 = round(r2_score(y_verify, y_verify_pred), 3)
verify_mae = round(mean_absolute_error(y_verify, y_verify_pred), 3)
verify_mse = round(mean_squared_error(y_verify, y_verify_pred), 3)
print('\nverify set(R2/MAE/MSE): ', verify_R2, '/', verify_mae, '/', verify_mse)
scatter_plot(y_verify, y_verify_pred, verify_R2, verify_mae, verify_mse)